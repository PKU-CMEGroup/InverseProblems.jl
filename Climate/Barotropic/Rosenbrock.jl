...
function check_rosenbrock_dimension(theta_dim::Int)
    theta_dim > 0 || error("theta_dim must be positive")
    iseven(theta_dim) || error("theta_dim must be even so it can be split into Rosenbrock blocks")
end

function rosenbrock_observation(theta_dim::Int)
    check_rosenbrock_dimension(theta_dim)
    y = zeros(Float64, theta_dim)
    y[2:2:end] .= 1.0
    return y
end
...
function rosenbrock_forward(theta::AbstractVector, c1::Float64)
    ...
    y_pred = zeros(Float64, theta_dim)
    for i in 1:2:theta_dim
        y_pred[i] = 10.0 * (theta[i + 1] - c1 * theta[i]^2)
        y_pred[i + 1] = theta[i]
    end
    return y_pred
end
...
function rosenbrock_reference_grid(;
    ...
    potential = 0.5 .* (
        100.0 .* (y_grid .- c1 .* x_grid.^2).^2 ./ c2 .+
        (1.0 .- x_grid).^2 ./ c2 .+
        (x_grid.^2 .+ y_grid.^2) ./ prior_std^2
    )
    ...
    mean_ref = repeat(mean_pair_ref, n_blocks)
    map_ref = repeat(map_pair_ref, n_blocks)
...
function rosenbrock_observation_model(c1::Float64, c2::Float64, prior_std::Float64, theta_dim::Int)
    ...
    sigma_y = Array(Diagonal(vcat(fill(c2, theta_dim), fill(prior_std^2, theta_dim))))
