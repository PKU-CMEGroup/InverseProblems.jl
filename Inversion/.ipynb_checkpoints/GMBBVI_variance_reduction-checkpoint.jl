...
function update_ensemble!(gmgd::BBVIObj{FT, IT}, func_Phi::Function, dt_max::FT, iter::IT, N_iter::IT) where {FT<:AbstractFloat, IT<:Int} #从某一步到下一步的步骤
    ...
    x_p_num[1] = Int(1)
    for i = 2 : N_modes + 1
        x_p_num[i] = Int(x_p_num[i-1] +  max(round(N_modes * N_ens * weight_x_p[i-1]), 50))
    end
    x_p = zeros(x_p_num[N_modes + 1] - 1, N_x)

    for im = 1:N_modes
        # generate sampling points subject to Normal(x_mean [im,:], xx_cov[im]), size=(N_ens, N_x)
        x_p[x_p_num[im]:x_p_num[im+1]-1, :] = construct_ensemble(x_mean[im,:], sqrt_xx_cov[im]; c_weights = nothing, N_ens = x_p_num[im+1]-x_p_num[im])
        # log_ratio[i] = logρ[x_p[i,:]] + log func_Phi[x_p[i,:]]
    end

    # store Gaussian_density_helper(x_mean[im,:], inv_sqrt_xx_cov[im], x_p[imm,i,:])
    GDS = zeros(N_modes, N_modes, maximum(x_p_num[im+1]-x_p_num[im] for im in 1 : N_modes)) # [im, imm, i]
    for im=1:N_modes, imm=1:N_modes
        for i = 1 : x_p_num[imm+1]-x_p_num[imm]
            GDS[im,imm,i]=Gaussian_density_helper(x_mean[im,:], inv_sqrt_xx_cov[im], x_p[i+x_p_num[imm]-1,:])
        end
    end

    log_ratio = zeros(x_p_num[N_modes + 1] - 1)
    for im = 1 : N_modes
        for i = 1 : x_p_num[im+1]-x_p_num[im]
            for imm = 1:N_modes
                log_ratio[i+x_p_num[im]-1] += exp(logx_w[imm]) * GDS[imm,im,i]
            end
            log_ratio[i+x_p_num[im]-1] = log(log_ratio[i+x_p_num[im]-1]) + func_Phi(x_p[i+x_p_num[im]-1,:])
        end
    end

    for im = 1 : N_modes
        log_ratio_m1 = zeros(N_modes,N_x)
        log_ratio_m2 = zeros(N_modes,N_x,N_x)

        # E[logρ+Phi]
        log_ratio_mean = zeros(N_modes)
        for imm = 1 : N_modes
            log_ratio_mean[imm] = mean(log_ratio[i+x_p_num[imm]-1] for i = 1 : x_p_num[imm+1]-x_p_num[imm])
        end
        
        # weights allocation
        weight = exp.(logx_w*(1))
        # weight[im] = 0
        # weight[im] = 1-sum(weight)

        # E[(x-m)(logρ+Phi)]
        log_ratio_m1 = mean((x_p[i+x_p_num[im]-1,:]-x_mean[im,:])*log_ratio[i+x_p_num[im]-1] for i=1:x_p_num[im+1]-x_p_num[im])
        for iim = 1 : N_modes
            # log_ratio_m1[iim,:] = mean((x_p[iim,i,:]) * ((log_ratio[iim,i] - log_ratio_mean[iim])* GDS[im,iim,i] / sum(weight[imm] * GDS[imm,iim,i] for imm = 1:N_modes)) for i=1:N_ens)
            log_ratio_m2[iim,:,:] = mean((x_p[i+x_p_num[iim]-1,:]-x_mean[im,:])*((x_p[i+x_p_num[iim]-1,:]-x_mean[im,:])'*((log_ratio[i+x_p_num[iim]-1] - log_ratio_mean[iim])
                * GDS[im,iim,i] / 
                sum(weight[imm] * GDS[imm,iim,i] for imm = 1:N_modes)) # this term can be 0.0, therefore the result can be NaN
                ) for i=1:x_p_num[iim+1]-x_p_num[iim])
            # if isnan(norm(log_ratio_m2[iim,:,:]))==true for i = 1 : N_ens   println(sum(weight[imm] * GDS[imm,iim,i] for imm = 1:N_modes))   end end
        end
        d_x_mean[im,:] = -log_ratio_m1
        # d_x_mean[im,:] = -sum(weight[iim]*log_ratio_m1[iim,:] for iim = 1:N_modes)
        d_xx_cov[im,:,:] = -sum(weight[iim]*log_ratio_m2[iim,:,:] for iim = 1:N_modes)
    ...
    dt =  min(dt_max,  (0.01 + (1.0 - 0.01)*cos(pi/2 * iter/N_iter)) / (maximum(matrix_norm))) # keep the matrix postive definite.
