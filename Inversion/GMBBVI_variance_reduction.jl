...
function update_ensemble!(gmgd::BBVIObj{FT, IT}, func_Phi::Function, dt_max::FT, iter::IT, N_iter::IT) where {FT<:AbstractFloat, IT<:Int} #从某一步到下一步的步骤
    ...
    GDS = zeros(N_modes, N_modes, N_ens) # [im, imm, i]
    for im=1:N_modes, i=1:N_ens, imm=1:N_modes
        GDS[im,imm,i]=Gaussian_density_helper(x_mean[im,:], inv_sqrt_xx_cov[im], x_p[imm,i,:])
    end

    log_ratio = zeros(N_modes, N_ens)
    for im = 1 : N_modes
        for i = 1:N_ens
            for imm = 1:N_modes
                log_ratio[im, i] += exp(logx_w[imm]) * GDS[imm,im,i]
            end
            log_ratio[im, i] = log(log_ratio[im, i])+func_Phi(x_p[im,i,:])
        end
    end
    ...
    for im = 1 : N_modes
        ...
        # E[(x-m)(logρ+Phi)]
        log_ratio_m1 = mean((x_p[im,i,:]-x_mean[im,:])*log_ratio[im,i] for i=1:N_ens)
        for iim = 1 : N_modes
            ...
            log_ratio_m2[iim,:,:] = mean((x_p[iim,i,:]-x_mean[im,:])*((x_p[iim,i,:]-x_mean[im,:])'*((log_ratio[iim,i] - log_ratio_mean[iim])
                * GDS[im,iim,i] / 
