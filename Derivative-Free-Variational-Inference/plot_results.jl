...
# --- Main Plotting Loop ---
fig, axes = PyPlot.subplots(nrows=length(dim_test), ncols=5, figsize=(32, 6 * length(dim_test)), dpi=100)
...
for (idx, N_x) in enumerate(dim_test)
    ...
    ax_row[1].set_title("Reference (Dim $N_x)")

    # ---------------------------------------------------------
    # Load Data & Prepare Average/Last structures
    # ---------------------------------------------------------
    
    # For Convergence Plot (Col 5)
    gm_me, gm_ce, gm_l1 = [], [], []
    wn_me, wn_ce, wn_l1 = [], [], []
    
    # For Density Plots (Col 2 & 3) - Only keep the LAST trial
    last_gm_data = nothing
    last_wn_data = nothing
    
    # For Marginal Plot (Col 4) - Accumulate AVERAGE
    x_marg = collect(range(-15, 15, length=400))
    y_gm_accum = zeros(length(x_marg)) # Accumulator for GMBBVI PDF
    gm_loaded_count = 0
    
    wn_samples_dim1_all = Float64[] # Accumulator for WALNUTS samples (Dim 1)
    ...
    for t in 0:(n_trials-1)
        ...
        if isfile(fn_gm)
            ...
            # Accumulate Marginal Density (Dim 1)
        ...
        if isfile(fn_wn)
            ...
            # Accumulate Marginal Samples (Dim 1 is index 0 in Python/NPZ array, row 1 here)
            # samples shape is (Dims, N_samples)
            samples = data["samples"]
            append!(wn_samples_dim1_all, samples[1, :])
    ...
    # ---------------------------------------------------------
    # 2. Our Method (GMBBVI) - Visualization of LAST Trial
    # ---------------------------------------------------------
    ...
    if last_wn_data !== nothing
        ...
        n_kde_samples = min(size(wn_samples, 2), 10000)
    ...
    ax_row[3].set_title("WALNUTS")

    # ---------------------------------------------------------
    # 4. Marginal Dim 1
    # ---------------------------------------------------------
    y_true = pdf.(Normal(0, 3.0), x_marg)
    ax_row[4].plot(x_marg, y_true, "k--", linewidth=2, label="True", alpha=0.6)

    # GMBBVI Marginal (Average)
    if gm_loaded_count > 0
        y_gm_avg = y_gm_accum ./ gm_loaded_count
        ax_row[4].plot(x_marg, y_gm_avg, color="red", linestyle="-", label="GMBBVI")
    end

    # WALNUTS Marginal (All Samples Histogram)
    if !isempty(wn_samples_dim1_all)
        ax_row[4].hist(wn_samples_dim1_all, bins=100, density=true, color="blue", alpha=0.2, label="WALNUTS")
    end
    
    ax_row[4].legend(fontsize=7, loc="upper right")
    ax_row[4].set_title("Marginal Dim 1")

    # ---------------------------------------------------------
    # 5. Convergence (Dim 1, Average, Distinct Colors)
    # ---------------------------------------------------------
    ax_err = ax_row[5]
    ...
    # GMBBVI Colors
    plot_err_with_std(ax_err, gm_me, "red",       "GM Mean", "-")
    plot_err_with_std(ax_err, gm_ce, "magenta",   "GM Var",  "-")
    plot_err_with_std(ax_err, gm_l1, "darkorange","GM L1",   "-")
    
    ax_err.set_xlabel("GMBBVI Iterations", color="black")
    ...
    if !isempty(wn_me)
        ...
        plot_err_with_std(ax_err_twin, wn_me, "blue",      "WN Mean", "-", x_wn)
        plot_err_with_std(ax_err_twin, wn_ce, "cyan",      "WN Var",  "-", x_wn)
        
        # if !isempty(wn_l1)
        #     len_l1 = length(wn_l1[1])
        #     x_wn_l1 = range(wn_chunk_size, stop=wn_total_iter, length=len_l1)
        #     plot_err_with_std(ax_err_twin, wn_l1, "forestgreen", "WN L1",   "-", x_wn_l1)
        # end
    end
    
    ax_err_twin.set_xlabel("WALNUTS Iterations", color="black")
    ...
    ax_err.legend(vcat(lines_1, lines_2), vcat(labels_1, labels_2), fontsize=6, loc="upper right", framealpha=0.9)
end
