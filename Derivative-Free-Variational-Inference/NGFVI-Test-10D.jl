...
N_ens = 2 * N_x + 1  #number of particles for MC in BBVI
...
func_V(x) = dPhi(x, func_args)
objs = [Gaussian_mixture_NGFVI(func_V, x0_w[1:N_modes], x0_mean[1:N_modes,:], xx0_cov[1:N_modes,:,:]; diagonal_covariance = false, N_iter = N_iter, dt = dt, N_ens = N_ens, stoch_updating_m_w = true)
        for N_modes in N_modes_array][1]
y_2d = y[1:2]
...
fig.tight_layout()
fig.savefig("NGFVI-10D-Multimodal.pdf")
