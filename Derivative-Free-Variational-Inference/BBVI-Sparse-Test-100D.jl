...
include("../Inversion/GMBBVI-Sparse.jl")
...
N_x = 20
N_r = 10 # a low rank
μ0, ϵ_init, Q_init = zeros(N_x), 0.2, 1*Diagonal(ones(N_x))[1:N_x, 1:N_r]

N_modes = N_modes_array[end]
x0_w  = ones(N_modes)/N_modes
x0_mean, ϵ0, Q0 = zeros(N_modes, N_x), zeros(N_modes), zeros(N_modes, N_x, N_r)

for im = 1:N_modes
    x0_mean[im, :] .= rand(MvNormal(zeros(N_x), Diagonal(ones(N_x)))) + μ0
    ϵ0[im] = ϵ_init
    Q0[im, :, :] .= Q_init
end
...
N_ens = N_r  #number of particles for MC in BBVI
updating_rule = "direct"
 updating_rule = "EM"
...
objs = [Gaussian_mixture_BBVI_Sparse(func_Phi, x0_w[1:N_modes], x0_mean[1:N_modes,:], ϵ0[1:N_modes], Q0[1:N_modes,:,:]; N_iter = N_iter, dt = dt, N_ens=N_ens, η = η, updating_rule = updating_rule)
        for N_modes in N_modes_array]
y_2d = y[1:2]
func_args = (y_2d, ση, A , Gtype)
visualization_2d_sparse(ax[1,:]; Nx = Nx, Ny = Ny, x_lim=[-7.0, 5.0], y_lim=[-4.0, 5.0], func_Phi=func_Phi, objs=objs, label="BBVI-Sparse")
# visualization_2d_sparse_G(ax[1,:]; Nx = Nx, Ny = Ny, x_lim=[-7.0, 5.0], y_lim=[-4.0, 5.0], func_Phi=func_Phi, objs=objs, label="BBVI-Sparse")
